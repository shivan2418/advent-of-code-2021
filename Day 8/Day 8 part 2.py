...
RealInout = """dfgabce cadfgb cefa ca aecbg dfcegb geabd ecbfg cab agcfbe | egbfadc dbgae gcfeb abgdfc
...
class String(str):
    ...
    def __sub__(self, other):
        # Override the subtraction behavior
        if isinstance(other, String):
            return String(self.replace(other, ""))  # Remove occurrences of 'other'
        else:
            raise TypeError("Subtraction can only be performed with another CustomString")
...
def make_digit_map_from_line(line:str) -> dict[str, str]:

    line = line.replace("|", " ").split(" ")
    line = ["".join(sorted(word)) for word in line if len(word)>0]
    ...
    for word in line:
        if len(word) in LEN_NUM_DICT.keys():
            DIGIT_MAP[word] = LEN_NUM_DICT[len(word)]


    REV_DIGIT_MAP = {v:set(k) for k,v in DIGIT_MAP.items()}

    lines = [line for line in word_sets if set(line) not in REV_DIGIT_MAP.values()]

    # map 3
    all_five_len = [line for line in lines if len(line)==5]
    find_3 = [line for line in all_five_len if REV_DIGIT_MAP['1'].issubset(set(line))]
    DIGIT_MAP[find_3[0]] ="3"
    REV_DIGIT_MAP = {v:set(k) for k,v in DIGIT_MAP.items()}

    # map 0
    all_six_len = [line for line in lines if len(line)==6]
    # exclude 6 and 4 as possibility
    find_0 = [line for line in all_six_len if REV_DIGIT_MAP['7'].issubset(set(line)) and not REV_DIGIT_MAP['4'].issubset(set(line))]
    DIGIT_MAP[find_0[0]] ="0"
    REV_DIGIT_MAP = {v:set(k) for k,v in DIGIT_MAP.items()}

    TOP_BAR = REV_DIGIT_MAP['7'].difference(REV_DIGIT_MAP['1'])
    MIDDLE_BAR = REV_DIGIT_MAP['8'].difference(REV_DIGIT_MAP['0'])
    TOP_LEFT_BAR = REV_DIGIT_MAP['4']-REV_DIGIT_MAP['1']-MIDDLE_BAR

    # map 9
    lines = [line for line in word_sets if set(line) not in REV_DIGIT_MAP.values()]
    all_six_len = [line for line in lines if len(line)==6]
    find_9 = [line for line in all_six_len if REV_DIGIT_MAP['1'].issubset(set(line))]
    DIGIT_MAP[find_9[0]] = "9"
    REV_DIGIT_MAP = {v: set(k) for k, v in DIGIT_MAP.items()}
    lines = [line for line in word_sets if set(line) not in REV_DIGIT_MAP.values()]

    # map 6
    all_six_len = [line for line in lines if len(line) == 6]
    DIGIT_MAP[all_six_len[0]] = "6"
    REV_DIGIT_MAP = {v: set(k) for k, v in DIGIT_MAP.items()}
    lines = [line for line in word_sets if set(line) not in REV_DIGIT_MAP.values()]
    ...
    # find top right bar
    TOP_RIGHT_BAR = REV_DIGIT_MAP['1'].difference(REV_DIGIT_MAP['6'])

    find_5 = [line for line in all_five_len if TOP_LEFT_BAR.issubset(set(line)) and TOP_BAR.issubset(set(line))]
    DIGIT_MAP[find_5[0]] = "5"
    REV_DIGIT_MAP = {v: set(k) for k, v in DIGIT_MAP.items()}
    lines = [line for line in word_sets if set(line) not in REV_DIGIT_MAP.values()]

    # map 2
    all_five_len = [line for line in lines if len(line) == 5]
    DIGIT_MAP[all_five_len[0]] = "2"

    return DIGIT_MAP

def solve_part_2():
    lines = parse_input(RealInout)
    numbers = []

    for line in lines:
        DIGIT_MAP = make_digit_map_from_line(line)
        signal_part = line.split("|")[-1].split(' ')

        nums = [DIGIT_MAP["".join(sorted(signal))] for signal in signal_part if signal != ""]

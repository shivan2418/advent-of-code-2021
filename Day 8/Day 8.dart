...
String TestInput1 = """dfgabce cadfgb cefa ca aecbg dfcegb geabd ecbfg cab agcfbe | egbfadc dbgae gcfeb abgdfc
...
List<String> parseInput(String input) {
  ...
  var linesSorted = [for (var x in lines) x.split("|").last];
  return linesSorted;
}

int tallyUniquePatterns(List<String> lines) {
  int count = 0;

  for (var line in lines) {
    for (var value in line.split(' ')) {
      var pattern = MapDigit(value);
      if (pattern != "") {
        count++;
      }
    }
...
main() {
  ...
  print(tallyUniquePatterns(input));
  print(input);

  var input1 = parseInput(TestInput1);
  print(tallyUniquePatterns(input1));

...
class NestedPair {
  ...
  void set_left_right(List<NestedPair> children) {
    if (children.length == 1) {
      return;
    }
    if (children.length == 2) {
      children.first.right = children.last;
      children.last.left = children.first;
    }

    if (children.length > 2) {
      children.forEachIndexed((int index, NestedPair child) {

        child.left = get_child(children, index-1);
        child.right = get_child(children, index+1);

      });
    }
  }

  get_child(List<NestedPair> children, int index) {
    var child = null;
    try {
